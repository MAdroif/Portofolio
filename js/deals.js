const Database = {
  ...
  async loadData() {
    try {
      ...
      alert(`Gagal mengambil data deals: ${e}`);
    }
  }
};
...
const Deals = {
  ...
  render(data) {
    ...
    if (statsContainer) {
      statsContainer.innerHTML = `
        <div class="deal-stat-card">
          <p class="deal-stat-label">Total Deals</p>
          <p class="deal-stat-value">${totalDeals}</p>
        </div>
        <div class="deal-stat-card">
          <p class="deal-stat-label">Total Value</p>
          <p class="deal-stat-value">Rp ${totalValue.toLocaleString('id-ID')}</p>
        </div>
        <div class="deal-stat-card">
          <p class="deal-stat-label">Avg. Deal Size</p>
          <p class="deal-stat-value">Rp ${Math.round(avgDealSize).toLocaleString('id-ID')}</p>
        </div>
        <div class="deal-stat-card">
          <p class="deal-stat-label">Win Rate</p>
          <p class="deal-stat-value ${winRate >= 50 ? 'won' : ''}">${Math.round(winRate)}%</p>
    ...
    this.stages.forEach(stage => {
      ...
      // Hitung total value per stage
      const stageTotal = deals.reduce((sum, deal) => sum + deal.value, 0)
      
      // Update header kolom dengan badge dan total value
      const colHeader = col.querySelector('.kanban-col-header');
      if (colHeader) {
        const badgeSpan = colHeader.querySelector('.col-badge');
        const totalSpan = colHeader.querySelector('.col-total');
        if (badgeSpan) badgeSpan.textContent = deals.length;
        if (totalSpan) totalSpan.textContent = `Rp
        ${stageTotal.toLocaleString('id-ID')}`;
      }
