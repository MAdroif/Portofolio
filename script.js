...
function enableEditMode(taskId, taskText) {
  isEditing = true;
...
function saveEdit() {
  const taskId = parseInt(editingTaskId.value);
    ...
    if (filteredTasks.length === 0) {
        ...
        filteredTasks.forEach(task => {
            ...
            editBtn.addEventListener('click', (e) => {
              e.stopPropagation();
              enableEditMode(task.id, task.taskText);
            });
...
function initEventListener() {
  ...
  editTaskBtn.addEventListener('click', saveEdit);
  // Event listener unuk Escape key (cancel edit)
  taskInput.addEventListener('keypress', (e) => {
